...
flowchart TB
    ...
    RA --> ACC["accommodation-research<br/>每個過夜鎮研究+查證住宿（設施/車位/洗衣）"]
    ACC --> LEG["inter-stop-legs<br/>城際交通（哪班車/末班車/Pass、自駕單日車程）"]
    LEG --> CAL["calendar-check<br/>查旅遊期間當地假日（含補假）"]
    CAL --> SEA["seasonal-advisory<br/>查季節/天氣危害（道路封閉、雪鏈、日照）"]
    SEA --> COST["cost-rollup<br/>加總住宿/交通/Pass + 雜支，對照預算"]
    COST --> SYN["itinerary-synthesis<br/>排出逐日行程 + 備案 + 行前清單<br/>（閉館日不排、假期標人潮）"]
...
- 流水線由 `skills/` 下的 15 個 skill 組成，全程由 `orchestrator` 調度。
