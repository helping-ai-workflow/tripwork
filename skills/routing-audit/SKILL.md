...
3. For each planned inter-district hop, estimate travel minutes (subway/taxi) and classify with `scripts/distance.py::classify_hop(mins, max_hop_mins, km=haversine_km(...), mode=..., source_url=...)`. Passing `km`+`mode` enables the physical-plausibility floor: an estimate below `scripts/distance.py::min_plausible_mins(km, mode)` (urban transit ≈ 15 km/h door-to-door) returns `implausible`. Record the corrected estimate **together with where it came from**: set `duration_source` to `map_estimate` (a routing engine's number) or `sourced_timetable` (a published schedule) — **and record `source_url` for either one**. A hop that clears the floor carrying only `agent_estimate` is `unsourced`, not `ok` — raising your own guess until it clears is not evidence; so is relabelling it `map_estimate`/`sourced_timetable` with no `source_url` to point at (I1) — the schema also rejects that combination. Use `trip-brief.routing.max_hop_mins` (default 60). For region-in-range checks, the region radius defaults to 5 km and is overridable via `trip-brief.routing.region_radius_km`.
