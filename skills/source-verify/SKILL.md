...
1. **Multi-source** (Gate 1) — >= 2 independent sources, at least one in the destination's local language. Pass `trip-brief.destination.local_lang` as `classify_candidate(..., local_lang=...)` so the local-language requirement is enforced for the right language. Fewer/wrong language -> `unverified`. This gate precedes geocode so a single-source candidate is never misclassified as `rejected`.
