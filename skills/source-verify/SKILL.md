...
**`cluster_fallback` is not a general-purpose escape.** When Nominatim cannot resolve a POI, the default outcome is D7 (`unverified`). Recording a cluster centroid instead keeps the POI `verified` ONLY when something independent of the coordinate proves the venue exists — three proofs are accepted: a source flagged `official: true`; the `gmaps_place_id` captured during the Gate 0 operating check (of plausible shape — a stray short value does not count); or a **sourced** `business_status` object (`{status, source_url, as_of}`, the same object Gate 0 above requires). The third is what makes this gate reachable on a keyless machine: Gate 0 documents only one route that yields a `gmaps_place_id` (the Places API), so a consumer without a key had exactly one possible proof — an official source — and `cluster_fallback`'s trigger population is precisely the small venues least likely to have one. A dated, sourced statement that the venue is operating is itself independent evidence that it exists, and Gate 0 already guarantees two keyless routes to it (a dated official/social statement, or a `tel:` phone confirmation), so accepting it here closes the asymmetry without inventing a fourth evidence vocabulary. The bare-string `business_status` stays rejected — self-attested, no `source_url`, no `as_of`, nothing to review; accepting it would reopen the bare-string hole Gate 0 above closed. **No pipeline stage writes `gmaps_place_id`** — it is recorded by the agent during Gate 0's Places-API route, or not at all; treat it as agent-authored, not driver-produced. Without one of the three proofs, `verify_poi` returns `unverified` — the centroid is the district's midpoint, so it satisfies the region check by construction and carries no `resolved_name` for the name-match check to compare, which means an unproven POI would clear three gates on a coordinate that describes an area, not a place.
